...
<p align="center">
  ...
  <h2 align="center" style="font-weight: 600">music</h2>
  ...
  <p align="center">
    ...
    <a href="https://t.me/music" target="blank"><strong>💬 加入交流群</strong></a>
    <br />
    <br />
  </p>
</p>
...
访问本项目的 [Releases](https://github.com/qier222/music/releases)
页面下载安装包。

- macOS 用户可以通过 Homebrew 来安装：`brew install --cask music`

- Windows 用户可以通过 Scoop 来安装：`scoop install extras/music`
...
```sh
git clone --recursive https://github.com/qier222/music.git
```
...
5. 复制 `/.env.example` 文件为 `/.env`，修改里面 `VUE_APP_NETEASE_API_URL` 的值为网易云 API 地址。本地开发的话可以填写 API 地址为 `http://localhost:3000`，music 地址为 `http://localhost:8080`。如果你使用了反向代理 API，可以填写 API 地址为 `/api`。
...
```sh
bash <(curl -s -L https://raw.githubusercontent.com/qier222/music/main/install-replit.sh)
```
...
2. 使用 `git clone --recursive https://github.com/qier222/music.git` 克隆本仓库到本地。
...
查看 Todo 请访问本项目的 [Projects](https://github.com/qier222/music/projects/1)
